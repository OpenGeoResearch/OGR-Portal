  ...
  <v-row no-gutters>
    ...
    >
      ...
      >
        <template v-slot:top>
          <v-toolbar
              flat
          >
            <v-toolbar-title>Audios</v-toolbar-title>
          </v-toolbar>
        </template>
        <template v-slot:item.audio.created="{ item }">
          {{ item.audio.created | dateParse('YYYY-MM-DD HH:mm') | dateFormat('DD.MM.YYYY - HH:mm') }}
        </template>
        <template v-slot:item.audio.url="{ item }">
          <audio
              controls
          >
            <source
                :src="item.audio.url + '?token=' + item.audio.token"
            />
      ...
      <v-dialog v-model="dialog.delete" max-width="500px">
        <v-card>
          <v-card-title class="text-h5">Audio wirklich löchen?</v-card-title>
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="primary" text @click="dialog.delete = false">Abbrechen</v-btn>
            <v-btn color="primary" text @click="deleteAudio()">Ja</v-btn>
            <v-spacer></v-spacer>
          </v-card-actions>
        </v-card>
      </v-dialog>
...
export default {
  ...
  data () {
    return {
      ...
      expanded: [],
      headers: [
        { text: 'ID', align: 'start', value: 'audio.id', sortable: false },
        { text: 'Zeitstempel', value: 'audio.created', sortable: false },
        { text: 'Vorschau', value: 'audio.url', sortable: false },
  ...
  methods: {
    approveAudio (item) {
      this.$apollo.mutate({
        mutation: gql`mutation ($fileUUID: String!) {
                        approveAudio(fileUUID: $fileUUID) {
                          audio {
                        id
                        fileUUID
                        questionUUID
        ...
        this.audios = response.data.approveAudio
      })
    },
    deleteDialog (item) {
      this.item = item
    ...
    deleteAudio () {
      ...
      this.$apollo.mutate({
        ...
        this.audios = response.data.deleteAudio
      })
      ...
      if (item.question.user !== null) {
        this.$apollo.mutate({
          mutation: gql`mutation ($fileUUID: String!, $username: String!) {
                banUserAudio(fileUUID: $fileUUID, username: $username) {
                  audio {
                    id
                    fileUUID
                    questionUUID
          ...
          this.audios = response.data.banUserAudio
        })
      }
