...
import DeCategories from '@/static/de_categories.json'
import EnCategories from '@/static/en_categories.json'
...
export function getCategoryFromCode (code, lang) {
  ...
  return (category !== undefined) ? category.label : 'Unbekannte Kategorie'
}
...
export function calculateTopHierarchicalCategories (topCategoriesList, lang) {
  const topCategoriesTmp = topCategoriesList.slice().sort((a, b) => { return (a.code > b.code) ? -1 : 1 })
  ...
  return Object.values(topCategoriesName).sort((a, b) => { return (a.count > b.count) ? -1 : 1 })
}
...
export function linkify (inputText, baseUrl) {
  ...
  text = text.replace(pattern1, '<a href="$&" target="_blank">Frage $1</a>')
